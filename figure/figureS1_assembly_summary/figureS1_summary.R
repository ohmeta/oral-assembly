...
dset <- read.table("../figure1_unkonwnness/00.data/sample_unknown_MAGs.tsv", header = T, sep = "\t")

p_MAG_study <- print(
  ggplot(dset, aes(x = studys, y = MAGs.HQ.MQ., fill = studys)) 
  ...
  + coord_flip())
ggsave("S1a_MAGs_in_studys.pdf", p_MAG_study, width = 8, height = 6)

#### c. The number of all SGBs (>0.001 abundance) for each samples. ####
profile_file = "../../../../abundance_profile_strain.jgi.tsv"
meta <- read.table("../figure5_geography/00.data/metadata.txt", header = T, sep = "\t")
...
figc.dset <- merge(SGB_sum.dset, meta[,c(1,2)], by.x = "Sample_ID", by.y = "Run_id")
p.c <- print(ggplot(figc.dset, aes(x = Country, y = SGB_num, fill = Country)) 
             + geom_violin()
...
p.d <- print(ggplot(figd.dset, aes(x = Country, y = uSGB_richnness, fill = Country)) 
             + geom_violin()
...
ggsave("uSGB_richnness_per_sample.pdf", p.d, width = 6, height = 6)
...
uSGB_sum.dset <- merge(usum.dset, meta[,c(1,2)], by.x = "Sample_ID", by.y = "Run_id")

p.e <- print(ggplot(uSGB_sum.dset, aes(x = Country, y = uSGB_abundance, fill = Country)) 
                    + geom_violin()
                    + guides(fill=FALSE)
                    + theme_bw(base_size = 16) 
                    + coord_flip())
ggsave("uSGB_abundance.pdf", p.e, width = 6, height = 6)
